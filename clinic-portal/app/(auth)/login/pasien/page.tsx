...
export default function PasienLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-700 flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm relative">
        <Link href="/login" className="inline-flex items-center gap-1.5 text-sm text-emerald-300/70 hover:text-emerald-200 mb-6 transition-colors">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white">Portal Pasien</h1>
          <p className="text-emerald-200/80 mt-1.5 text-sm">Masuk ke akun pasien aftech Klinik</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-8 border border-white/10 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input type="email" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-emerald-300/50 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-all"
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-white hover:bg-emerald-50 text-emerald-800 font-bold py-3.5 rounded-xl transition-all shadow-lg hover:shadow-xl disabled:opacity-50">
              {loading ? (
          ...
          <p className="text-center text-sm text-emerald-200/60 mt-6">
            Belum punya akun?{' '}
            <Link href="/register" className="text-emerald-300 font-medium hover:text-emerald-200 transition-colors">Daftar di sini</Link>
          </p>
        </div>
      </div>
    </div>
